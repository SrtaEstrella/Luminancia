...
function onViewChange(){const v=parseInt(document.getElementById('viewMode').value);window._viewStrip=isNaN(v)?-1:v;scheduleRender();}

function loadRef(e){const file=e.target.files[0];if(!file)return;document.getElementById('refName').textContent=file.name;document.getElementById('clearRefBtn').style.display='';const reader=new FileReader();reader.onload=function(ev){const img=new Image();img.onload=function(){refImage=img;document.getElementById('ref-col').style.display='';updateRefCanvas();applyZoom();};img.src=ev.target.result;};reader.readAsDataURL(file);}
function clearRef(){refImage=null;document.getElementById('ref-col').style.display='none';document.getElementById('refFile').value='';document.getElementById('refName').textContent='';document.getElementById('clearRefBtn').style.display='none';}
...
function drawMarkerOnRefCv(){const refCv=document.getElementById('refCv');if(window._markerPct==null)return;const ctx=refCv.getContext('2d'),my=pctToY(window._markerPct,refCv.height);ctx.strokeStyle='#4a80c0';ctx.lineWidth=2;ctx.setLineDash([6,3]);ctx.beginPath();ctx.moveTo(0,my);ctx.lineTo(refCv.width,my);ctx.stroke();ctx.setLineDash([]);const txt=window._markerPct.toFixed(1)+'%';ctx.font='bold 11px Inter,-apple-system,BlinkMacSystemFont,sans-serif';ctx.fillStyle='#4a80c0';ctx.textBaseline='bottom';ctx.fillText(txt,refCv.width-60,my-4);}
...
function clearMarker(){window._markerPct=null;renderPreview(window._viewStrip);if(refImage)updateRefCanvas();}

let _lang='zh';
function toggleLang(){_lang=_lang==='zh'?'en':'zh';switchLang(_lang);document.getElementById('langBtn').textContent=_lang==='zh'?'中':'EN';document.getElementById('langBtn').style.fontSize=_lang==='zh'?'16px':'12px';buildSegmentUI();buildColorUI();document.getElementById('resetBtn').textContent=T.reset;document.getElementById('exportImgBtn').textContent=T.exportImg;document.getElementById('exportJsonBtn').textContent=T.exportJSON;document.getElementById('importJsonBtn').textContent=T.importJSON;document.getElementById('fitBtn').textContent=T.fitBtn;document.getElementById('clearMarkerBtn').textContent=T.clearMarkerBtn;document.getElementById('clearMarkerBtn').title=T.clearMarkerTitle;document.getElementById('uploadBtn').textContent=T.upload;document.getElementById('clearRefBtn').textContent=T.clear;document.getElementById('footer').textContent=T.footer;document.getElementById('previewLabel').textContent=T.preview;document.getElementById('refImageLabel').textContent=T.refImage;document.getElementById('leftSeqLabel').textContent=T.leftSeq;document.getElementById('rightSeqLabel').textContent=T.rightSeq;const vm=document.getElementById('viewMode');vm.innerHTML='<option value="all">'+T.allStrips+'</option>';for(let i=0;i<6;i++)vm.innerHTML+='<option value="'+i+'">'+T.stripN+(i+1)+'</option>';scheduleRender();}
function toggleTheme(){const r=document.documentElement;r.classList.toggle("light");const isLight=r.classList.contains("light");document.getElementById("themeBtn").textContent=isLight?T.themeLight:T.themeDark;}
